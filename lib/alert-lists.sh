...
display_alerts_by_version_comparison() {
    ...
    echo "$alerts_json" | jq -r "map(select(.$filter_field == true)) | map(. + {
    ...
    }) | sort_by(.severity_order) | .[] | \"\(.security_advisory.severity)|\(.security_advisory.summary)|\(.dependency.package.name)|\(.security_vulnerability.first_patched_version.identifier)\"" | while IFS='|' read -r severity summary package version; do
