...
fix_vulnerabilities() {
    ...
    case $pm in
        ...
        "yarn")
            ...
            done < <(echo "$alerts_json" | jq -c 'map(select(.security_vulnerability.first_patched_version.identifier != null and (.is_blocked != true or .blocked_reason != "dependabot_timed_out"))) | .[]')
